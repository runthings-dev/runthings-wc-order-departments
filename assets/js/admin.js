            ...
            if (xhr.responseText && xhr.responseText.indexOf('<term_id><![CDATA[') !== -1) {
                ...
                if (termIdEnd > termIdStart) {
                    ...
                    if (termId && !isNaN(termId) && parseInt(termId) > 0) {
                        ...
                        $('#' + runthingsOrderDepartments.metaPrefix + 'department_categories').val(null).trigger('change');
                        $('#' + runthingsOrderDepartments.metaPrefix + 'selected_products').val(null).trigger('change');
                    }
                }
            }
        }
